...
/**
 * <code>TreeNode</code> objects can be combined into a tree. The tree is not a
...
public class TreeNode {
	 ...
	 */
	public TreeNode findParent(TreeNode parentNode, Long id) {      
		TreeNode correctNode = null ;   
		Object object[] = (Object[]) parentNode.getUserObject();
		Long myid = (Long) object[0];
		if (myid.equals(id)){  
	        correctNode = parentNode ;   
	    }   
	    else {   
	    	TreeNode[] childNodes = parentNode.children();      
	        for ( TreeNode child : childNodes ) {        
	            correctNode = findParent( child, id ) ;   
	            if( correctNode != null ) {   
	                break ;   
	            }   
	        }        
	 ...
	 *
	 * @param node            is the node to recursively check for its depth. This should be
	 ...
	 * @param doc the doc
	 */
	public static void printTree(TreeNode doc) {
		if (doc == null) {
		...
		try {
			Object object[] = (Object[]) doc.getUserObject();
			System.out.println(object[0] + "  " + object[2]);

			TreeNode[] ch = doc.children();
