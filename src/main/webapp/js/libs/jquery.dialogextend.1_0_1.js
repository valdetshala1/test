...
(function($){
	...
	var methods = {
		...
		"maximize" : function(){
			...
			$(self)
				...
				.dialog("widget")
					...
					.find(".ui-widget-header").css("class", "dialog-header").end()
					//.find(".ui-widget-header").css("background", "darkgreen").end()
		...
		"minimize" : function(){
			...
			$(self)
				...
				// make title-bar no-wrap
				.hide()
				//.dialog("widget").find(".ui-widget-header").css("background", "yellow")
				.dialog("widget")
					.find(".ui-dialog-buttonpane:visible").hide().end()
					.find(".ui-dialog-titlebar").css("white-space", "nowrap").end()
					.find(".ui-widget-header").css("background", "yellow").end()
				.find(".ui-dialog-content")
				// disable draggable-handle (for <titlebar=none> only)
				.dialog("widget")
					//.draggable("option", "handle", null)
			...
			var pos = minimizecount * 60;
			$(self).dialog('option', 'position', [1,pos]);
			minimizecount++;
		...
		"restore" : function(){
			var self = this;
			$(self).dialog("widget").find(".ui-widget-header").css("background", "#5a8354").end();
			// start!
			$(self)
				// trigger custom event
		...
		"_initButtons" : function(){
			...
			//$(titlebar).find(".ui-dialog-titlebar-close").remove();
			//$(titlebar).find(".ui-icon-closethick").remove();
			...
			*/
			...
			$(buttonPane)
				...
				//.append('<a class="ui-dialog-titlebar-close ui-corner-all" title="Close" href="#"><span class="ui-icon '+settings.icons.close+'">close</span></a>')
				.append('<a class="ui-dialog-titlebar-close ui-corner-all" title="Close" style="position: relative; margin:0px; top: 0px; width: 19px; padding: 1px; height: 18px; " href="#"><span class="ui-icon ui-icon-close" style="margin:2px; -1px; width: 19px; padding: 1px; height: 18px; ">close</span></a>')
				.append('<a class="ui-dialog-titlebar-maximize ui-corner-all" title="Maximize" href="#"><span class="ui-icon '+settings.icons.maximize+'">maximize</span></a>')
				.append('<a class="ui-dialog-titlebar-restore ui-corner-all" title="Restore" href="#"><span class="ui-icon '+settings.icons.restore+'">restore</span></a>')
				.append('<a class="ui-dialog-titlebar-minimize ui-corner-all" title="Minimize" href="#"><span class="ui-icon '+settings.icons.minimize+'">minimize</span></a>')
				
				
				// add effect to buttons
				.find(".ui-dialog-titlebar-maximize,.ui-dialog-titlebar-minimize,.ui-dialog-titlebar-restore,.ui-dialog-titlebar-close")
					.attr("role", "button")
		...
		"_initStyles" : function(){
			...
			if ( !$(".dialog-extend-css").length ) {
				...
				style += '.ui-dialog .ui-dialog-titlebar-close { width: 19px; padding: 1px; height: 18px; }';
				style += '.ui-dialog .ui-dialog-titlebar-restore { width: 19px; padding: 1px; height: 18px; }';
				style += '.ui-dialog .ui-dialog-titlebar-close span,';
				style += '.ui-dialog .ui-dialog-titlebar-maximize span,';
				style += '.ui-dialog .ui-dialog-titlebar-minimize span,';
				style += '.ui-dialog .ui-dialog-titlebar-restore span { display: block; margin: 1px; }';
				style += '.ui-dialog .ui-dialog-titlebar-close:focus,';
				style += '.ui-dialog .ui-dialog-titlebar-close:hover { padding: 0; border: none !important; background-position: center center; background-repeat: no-repeat;},';
		...
		"_initTitleBar" : function(){
			...
			switch ( settings.titlebar ) {
				...
				case "transparent":
					...
					//$(self).dialog("widget").find(".ui-widget-header").css("background", "darkgreen").end();
					/*
					 * 
					 *     background: #5a8354;
    ...
    color: #e8efe7; 
					 */
					$(self).dialog("widget").find(".ui-widget-header")
					.css({
							"background" : "#5a8354;-ms-linear-gradient(bottom, #5a8354, #8fb489);-moz-linear-gradient(center bottom, #5a8354 0, #8fb489 100%);-o-linear-gradient(#8fb489, #5a8354)",
							"filter" : "progid:DXImageTransform.Microsoft.gradient(startColorstr='#8fb489', endColorstr='#5a8354', GradientType=0)",
							"color" : "#e8efe7"
						}).end();
					$(self).dialog("widget").find(".ui-widget-header").css("background", "#5a8354").end();
					...
					/*
					$(self).dialog("widget").find(".ui-widget-header").css("background", "#5a8354").end();
					$(self).dialog("widget").find(".ui-widget-header").css("background", "-webkit-gradient(linear, left bottom, left top, color-stop(0, #5a8354), color-stop(1, #8fb489))").end();
					$(self).dialog("widget").find(".ui-widget-header").css("background", "-ms-linear-gradient(bottom, #5a8354, #8fb489)").end();
					$(self).dialog("widget").find(".ui-widget-header").css("background", "-o-linear-gradient(#8fb489, #5a8354)").end();
					$(self).dialog("widget").find(".ui-widget-header").css("filter", "progid:DXImageTransform.Microsoft.gradient(startColorstr='#8fb489', endColorstr='#5a8354', GradientType=0)").end();
					$(self).dialog("widget").find(".ui-widget-header").css("color", "#e8efe7").end();
					*/
					//$(self).dialog("widget").find(".ui-widget-header").addClass("dialog-header").end();
					
					//.find(".ui-widget-header").css("background", "darkgreen").end()
					//.find(".ui-widget-header").css("background", "darkgreen").end()
					//$(".ui-dialog").find(".ui-widget-header").css("background", "darkgreen");
					//$(self).dialog("widget").find(".ui-widget-header").css("background", "darkgreen");
					break;
				default:
