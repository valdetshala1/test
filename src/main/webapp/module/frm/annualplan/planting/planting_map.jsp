...
function loadPlanFeature(plantId){
	var url ='MUnitMapJson.plantPlanFeature.do?plant_id=' +plantId;
	$.ajax({
		type : 'GET', // GET or POST
		url : url,  
		async : false,
...
function loadImplementFeature(imId){
	var url ='MUnitMapJson.plantImplementFeature.do?imId=' +imId;
	$.ajax({
		type : 'GET', // GET or POST
		url : url,  
		async : false,
...
function executeSplit(poly, line) {
	...
	var link='';
	if($("#planting\\.activityStatus\\.status").val()=='APPROVED'){
...
function onPlan(feature) {
	...
	$geometry = null;
	$area = null;
	var impl=$("#planting\\.activityStatus\\.status").val()=='APPROVED';
	var text="POLYGON is out of stand!!";
	if(impl){
		$geometry = jQuery('#planting_declare').jqGrid ('getCell', rowId, 'im_geom');
		$area = jQuery('#planting_declare').jqGrid ('getCell', rowId, 'im_area');
		...
		var polygonFeature = wkt.read(wkt_obj);
     	...
     	if($("#planting\\.activityStatus\\.status").val()=='APPROVED'){
     		...
     		if(result!='false' && rowId>implNumber){
				result = checkImplOverlap(polygonFeature.geometry,$("#planting\\.id").val(),rowId);
     	...
     	if(result=='false'){
     		$.SmartMessageBox({
				title : "<i class='fa fa-sign-out txt-color-orangeDark'></i> <span class='txt-color-orangeDark'>"+text+"</span> ?",
//				content : $.logoutMSG || "<s:text name='stand.map.overlaped'/><br>"+text ,
				buttons : '[Ok]'
			});
     		if(impl ){
     			 jQuery('#planting_declare').jqGrid('setCell', rowId, 'im_geom', null);
	           	 jQuery('#planting_declare').jqGrid('getLocalRow', rowId).im_geom = null;
	           	 
	           	 jQuery('#planting_declare').jqGrid('setCell', rowId, 'im_area', null);
	           	 jQuery('#planting_declare').jqGrid('getLocalRow', rowId).im_area = null;
         	}else{
     		 	$geometry.val(null);	
     			$area.val(null);
         	}
             ...
             if(impl ){
                 if(rowId>implNumber){
	            	 $edit=jQuery('#planting_declare').jqGrid ('getCell', rowId, 'edit');
	            	 if($edit!='false'){
		            	 jQuery('#planting_declare').jqGrid('setCell', rowId, 'im_geom', polygonFeature.geometry);
		            	 jQuery('#planting_declare').jqGrid('getLocalRow', rowId).im_geom = polygonFeature.geometry;
		            	 
		            	 area = polygonFeature.geometry.getArea();
		            	 var fSiper = (area/10000).toFixed(4);
		            	 jQuery('#planting_declare').jqGrid('setCell', rowId, 'im_area', fSiper);
		            	 jQuery('#planting_declare').jqGrid('getLocalRow', rowId).im_area = fSiper;
	            	 }
                 }
             }else{
	       	 	area = polygonFeature.geometry.getArea();
...
function checkImplOverlap(polygon,planId,row_id){
	var result='';
	var implArray = $("#planting_declare").jqGrid('getRowData');
	var jsonText = JSON.stringify(implArray);
	if(jsonText!='' && polygon!=''){
		$.ajax({
			type : "GET", // GET or POST
			url : "Input.checkImplOverlap.do?implArray="+jsonText+"&polygon="+polygon+"&planId="+planId+"&rowId="+row_id, // the file to call
			async : false,
			beforeSend:function(jqXHR, settings) {
			},
			error : function(errorThrown, status, jqXHR) { // on error..
...
$.ajax({
	type : "GET", // GET or POST
	url : "FrmGrid.layerGisData.do?module=PLT", // the file to call
	async : false,
	beforeSend:function(jqXHR, settings) {
	},
	error : function(errorThrown, status, jqXHR) { // on error..
...
//                scales: [74153,50000,32957,21971,14648,9765,6510,4340,2893,1929,1286,857,572,381,254,169],
	...
	var openstreetmap = new OpenLayers.Layer.OSM();
        ...
        var plant_pl = new OpenLayers.Layer.WMS(
                "Pl Planting", geoserver_url,
                {
                    LAYERS: 'kfis:planting_pl_declare',
                    STYLES: '',
                    format: format,
                    tiled: true,
    				transparent: true,
      ...
      var plant_im = new OpenLayers.Layer.WMS(
                "Im Planting", geoserver_url,
                {
                    LAYERS: 'kfis:planting_impl_declare',
                    STYLES: '',
                    format: format,
                    tiled: true,
    				transparent: true,
      ...
      //  map_a.addLayers([openstreetmap, /* google_maps, google_satellite, google_terrain, */orto, vectorLayer,vectorLayerPlan, /*pedolo, mu,*/ compartments, stand,/*biodiv, plots,*/fmr,plant_pl,plant_im]);
      //ms
        drawLayer();
