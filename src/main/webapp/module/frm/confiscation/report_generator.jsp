...
// 			$( "#municipalityId" ).val(user_mun);
	...
	$("#cancelR").on('click', function(){
		$("#confiscation_report_generator").dialog("close");
	});
	$("#confiscation_report_generator").on('remove', function(){
		$("#confiscation_print").removeClass("ui-state-disabled");
	});
	$("#generator").on('click', function () {
		if($("#periodStart").val() && $("#periodEnd").val()){
			var parameters = "p_mun_id="+$("#municipalityId").val()+"&p_mun_name="+$("#municipality").val()+"&p_period_start="+$("#periodStart").val()+"&p_period_end="+$("#periodEnd").val()+"&p_region_id="+$("#planRegionId").val()+"&p_user="+$("#user").val();	
			//var link="frm/confiscation/"+activityName+"_confiscation_report.rptdesign";
			var link="\\frm\\confiscation\\"+activityName+"_confiscation_report.rptdesign";
