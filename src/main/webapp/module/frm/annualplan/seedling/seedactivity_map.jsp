...
function loadPlanFeature(seed_id){
	if(seed_id){
		var url ='MUnitMapJson.seedPlanFeature.do?seed_id=' +seed_id;
		$.ajax({
...
function loadImplementFeature(seed_id){
	var url ='MUnitMapJson.seedImplFeature.do?seed_id=' +seed_id;
	$.ajax({
		type : 'GET', // GET or POST
		url : url,  
		async : false,
...
function onPlan(feature) {
	...
	$geometry = null;
	var text="";
	if($("#seedActivity\\.activityStatus\\.status").val()=='APPROVED'){
		$geometry = $("#implementPlanedGeom");
	...
	if($geometry){
		var polygonFeature = wkt.read(wkt_obj);
     	...
     	if($("#seedActivity\\.activityStatus\\.status").val()=='APPROVED'){
     		...
     		text='POLYGON is out of planned area!!!';
       	}else{
       		result = isInsideState(polygonFeature.geometry);
       		text = 'POLYGON is out of state!!!';
       		if(result=='true'){
       			result = checkPlanOverlapping($("#activity\\.id").val(),$("#annualplanId").val(),polygonFeature.geometry);
       			text = 'POLYGON overlaps other activities!!!';
       			result+='';
           	}
        }
     	if(result=='false'){
     		$.SmartMessageBox({
				title : "<i class='fa fa-sign-out txt-color-orangeDark'></i> "+text,
				buttons : '[Ok]'
			});	
     		$geometry.val(null); 
         }else {
...
function isInsideState(polygon){
	var result='';
	if(polygon!=''){
		$.ajax({
			type : "GET", // GET or POST
			url : "Input.isInsideState.do?polygon="+polygon, // the file to call
			async : false,
			beforeSend:function(jqXHR, settings) {
			},
...
function checkPlanOverlapping(activity_id,annualplan_id,polygon){
	var result='';
	if(annualplan_id!='' && polygon!=''){
		$.ajax({
			type : "GET", // GET or POST
			url : "Input.checkPlanOverlapping.do?activity_id="+activity_id+"&polygon="+polygon+"&annualplan_id="+annualplan_id, // the file to call
			async : false,
			beforeSend:function(jqXHR, settings) {
			},
			error : function(errorThrown, status, jqXHR) { // on error..
...
$.ajax({
	type : "GET", // GET or POST
	url : "FrmGrid.layerGisData.do?module=SEED", // the file to call
	async : false,
	beforeSend:function(jqXHR, settings) {
	},
	error : function(errorThrown, status, jqXHR) { // on error..
...
function init(){
	...
	map_a = new OpenLayers.Map ('map_seed', options, {
		controls:[
