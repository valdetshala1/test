...
<!-- widget div-->
<div id="create_notification_form_content">
	...
	<div class="widget-body">
		<form id="create_notification_form" class="form-horizontal" action="PPRCNotificationSave.do" accept-charset="utf8" method="post">
				<input id="tenderRequest.id" name="tenderRequest.id" value="<s:property value='tenderRequest.id'/>" type="hidden">
				<input id="tenderRequest.createdDate" name="tenderRequest.createdDate" value="<s:property value='tenderRequest.createdDate'/>" type="hidden">
				...
				<input id="tenderStatus.id" name="tenderStatus.id" value="<s:property value='tenderStatus.id'/>" type="hidden">
 				<input id="tenderStatus.status" name="tenderStatus.status" value="<s:property value='publish.getDOMVALUE()'/>" type="hidden">
				...
				<s:if test="fieldErrors.get('tenderStatus.comment')!=null">
					<div class="form-group has-error">
					      <label class="col-md-2 control-label"><s:text name='tender.request.analyse.comment'/>:</label>
					 	  <div class="col-md-10">
					 	  		<textarea id="tenderStatus.comment" name="tenderStatus.comment" class="form-control" placeHolder="Enter the question!"></textarea>
			...
			<div class="form-actions">
				<div class="row">
					<div class="col-md-12">
						<button id="cancelNotification" class="btn btn-default" type="button">
							<s:text name="label.button.cancel"></s:text>
						</button>
						<button id="saveNotification" class="btn btn-primary" type="submit">
...
function getFileData(myFile){
	...
	$(document).ready(function () {
		...
		$("#cancelNotification").on('click', function () { 
			$("#dialogPPRCNotification").dialog("close");               
			return false;        
		});

		$("#saveNotification").on('click', function () { 
			$('#create_notification_form').submit();
			return false;        
		});

	/**	$("#create_notification_form").submit(function() {
			var this_form = $(this);
			$.ajax({
				...
				success : function(data, status, jqXHR) { // on success..
					$("#create_notification_form_content").html(jqXHR.responseText);
		...
		(function() {
			$('form').ajaxForm({
				...
				error : function(response, status, err){
					$("#create_notification_form_content").html(response);
					alert("error:" + errorThrown + ", status:" + status);
				},	    
