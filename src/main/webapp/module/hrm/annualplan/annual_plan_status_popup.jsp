...
<jsp:include page="../../base/validation.jsp"></jsp:include>
	...
	<div class="widget-body" id="tender_status_contenet">
		<form id="annual_plan_status" class="form-horizontal" action="HrmAnnPlanStatus.do" accept-charset="utf8" method="post">
			<fieldset>
							...
							<div class="form-group has-error">
							      <div class="col-md-10">
									<label class="col-md-4 control-label">
										<input type="radio" class="radiobox" id="apr" name="status.status" value="APPROVED">
										<span><s:text name="frm.statuses.approve"/></span>
									</label>
									<label class="col-md-4 control-label">
										<input type="radio" class="radiobox" id="mid" name="status.status" value="MIDYEAR">
										<span><s:text name="hrm.statuses.midyear.eval"/></span>
									</label>
									<label class="col-md-4 control-label">
										<input type="radio" class="radiobox" id="year" name="status.status" value="YEAREVALUATION">
										<span><s:text name="hrm.statuses.year.eval"/></span>
									</label>
								</div>
						     </div>
<%-- 						     <span class="help-block form-group has-error"><i class="fa fa-warning"></i> <s:property value="fieldErrors.get('status.status')"/> </span> --%>
			...
			<div class="">
				...
				<div class="form-group">
					  ...
					  <div class="col-md-10">
						<input id="fileUpload" name="fileUpload" class="form-control" type="file" multiple=true data-preview-file-type="any" data-upload-url="#">			  
					  </div>
			  	</div>
				<!-- END doc.file -->
		...
		(function() {
			$('form').ajaxForm({
				...
				error : function(response, status, err){
					$("#annual_plan_status").html(response);
			    ...
			    success : function(data, status, jqXHR) {
					...
					if (errors_exits == "true") {
						...
						$("#annual_plan_grid").trigger('reloadGrid');
						$("#annual_plan_status_content").dialog('close');
						var link = "HrmAnnPlan.form.do?" ;
