...
<!-- widget div-->
<div id="employee_certification_form_content">
	...
	<div class="widget-body">
		<form id="employee_certification_form" class="form-horizontal" action="EmployeeCertificationSave.do" accept-charset="utf8" method="post" enctype="multipart/form-data">
			...
			<input type="hidden" id="employeeSkill.docName" name="employeeSkill.docName" value="<s:property value="employeeSkill.docName"/>" />	
				...
				<s:if test="fieldErrors.get('employeeSkill.skillName')!=null">
					<div class="form-group has-error">
						<label class="col-md-2 control-label">*<s:text name="hrm.skill.label.certification.name"/></label>
						<div class="col-md-10">
							<input id="employeeSkill.skillName" name="employeeSkill.skillName" class="form-control" placeholder="<s:text name="hrm.skill.label.certification.name"/>" type="text">
				...
				<s:else>
					<div class="form-group">
						<label class="col-md-2 control-label">*<s:text name="hrm.skill.label.certification.name"/></label>
						<div class="col-md-10">
							<input id="employeeSkill.skillName" name="employeeSkill.skillName" value="<s:property value='employeeSkill.skillName'/>" class="form-control" placeholder="<s:text name="hrm.skill.label.certification.name"/>" type="text">
						</div>									
					</div>
				</s:else>
				...
				<div class="form-group">
					...
					<div class="col-md-10">
						<textarea class="form-control" placeholder="<s:text name="hrm.skill.label.details"/>"  name="employeeSkill.details" rows="4"><s:property value="employeeSkill.details"/></textarea>
					</div>
				</div>
			<!-- End employeeSkill.details -->
			
			<footer class="form-actions">
				<button id="cancelCer" class="btn btn-default" type="button">
					<s:text name="label.button.cancel"></s:text>
				</button>
				<button id="saveCer" class="btn btn-primary ladda-button" type="submit" data-style="expand-right" data-color="green" data-size="medium" data-spinner-size="25" data-spinner-color="#ff0000">
...
function getFileData(myFile){
	...
	$(document).ready(function () {
		...
		$("#cancelCer").on('click', function(){
			$("#employee_certification_form_content").dialog('close');
		});
// 		$("#saveCer").on('click', function(){
// 			$('#employee_certification_form').submit();
// 			});

		$("#employee_certification_form_content").on('remove',function(){
			$("#addCertificationButton").prop('disabled',false);
		});	
		Ladda.bind('#saveCer');
		(function() {
			$('form').ajaxForm({
				...
				error : function(response, status, err){
					alert(err);
					$("#employee_certification_form_content").html(response);
				},	    
			    success : function(data, status, jqXHR) {
			    	$("#employee_certification_form_content").html(jqXHR.responseText);
			    	var errors = jqXHR.getResponseHeader("errors_exits");
					if(!errors){
						var link = "Employee.certification.do?employeeId=" + $("#employeeId").val();
						loadURL(link, $("#certificationTab"));
						$("#employee_certification_form_content").dialog("close");		
