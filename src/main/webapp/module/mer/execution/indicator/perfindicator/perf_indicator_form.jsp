...
<div>
	...
	<!-- widget content -->
	<div class="widget-body" id="indicator_perf_form_content">
		<form id="indicator_perf_form" class="form-horizontal" action="PerfIndicatorSave.do" accept-charset="utf8" method="post" enctype="multipart/form-data">
			...
			<input type="hidden" id="indicator.type" name="indicator.type" value="P" />
				...
				<s:if test="fieldErrors.get('indicator.note')!=null">
					<div class="form-group has-error">
						...
						<div class="col-md-10">
							...
							<span class="help-block"><i class="fa fa-warning"></i> <s:property value="fieldErrors.get('indicator.note')"/> </span>
						</div>					
					</div>
				</s:if>
				<s:else>
		...
		<!-- end widget div -->
			<div class="form-actions">
				<div class="row">
					<div class="col-md-12">
						<button id="cancelPerfIndic" class="btn btn-default" type="button">
							<s:text name="label.button.cancel"></s:text>
						</button>
						<button id="savePerfIndic" class="btn btn-primary ladda-button" type="submit" data-style="expand-right" data-color="green" data-size="medium" data-spinner-size="25" data-spinner-color="#ff0000">
...
$(document).ready(function () {
		...
		Ladda.bind('#savePerfIndic');
		$("#savePerfIndic").on('click', function () {
			$("#indicator_perf_form :button").prop('disabled','disabled');                 
			$('#indicator_perf_form').submit();
			return false;        
		});

		$("#cancelPerfIndic").on('click', function() {
			$.ajax({
				type : "GET",
				url : "PerfIndicator.form.do",
				async : false,
				...
				success : function(data, status, jqXHR) { // on success..
					$("#indicator_perf_form_content").html(jqXHR.responseText);
					$.smallBox({
		...
		(function() {
			$('form').ajaxForm({
				...
				error : function(response, status, err){
					$("#indicator_perf_form_content").html(response);
					alert("error:" + errorThrown + ", status:" + status);
				},	    
			...
			$("#indicator_perf_form :button").prop('disabled',false);
		})();			
});
