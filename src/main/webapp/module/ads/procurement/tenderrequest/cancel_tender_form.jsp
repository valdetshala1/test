...
<!-- widget div-->
<div id="cancel_tender_form_content">
	...
	<div class="widget-body">
		<form id="cancel_tender_form" class="form-horizontal" action="CancelTenderSave.do" accept-charset="utf8" method="post">
			...
			<div class="form-actions">
				<div class="row">
					<div class="col-md-12">
						...
						<button id="saveT"  class="btn btn-primary ladda-button" type="submit" data-style="expand-right" data-color="green" data-size="medium" data-spinner-size="25" data-spinner-color="#ff0000">
...
function getFileData(myFile){
	...
	$(document).ready(function () {
		$("#cancelT").on('click', function () { 
			$("#cancel_tender_form_content").dialog("close");               
			return false;        
		});
		var reqStatus="";
		$("#saveT").on('click', function () { 
			$.SmartMessageBox({
					title : "<s:text name="tender.request.canceling.action"/>", 
					...
					if (ButtonPressed === "Yes") {
						...
						$("#cancel_tender_form :button").prop('disabled','disabled');
						$('#cancel_tender_form').submit();
					}
				});
		...
		(function() {
			$('form').ajaxForm({
				...
				error : function(response, status, err){
					$("#cancel_tender_form_content").html(response);
					alert("error:" + errorThrown + ", status:" + status);
				},	    
			    success : function(data, status, jqXHR) {
					$("#cancel_tender_form_content").html(jqXHR.responseText);
					...
					if (errors_exits == "true") {
						...
						//jQuery("#cancel_tender_grid").trigger("reloadGrid");
						getTenderRequestByStatus(reqStatus);
						if(reqStatus=="CAN"){
						$("#rb_can").prop('checked','true');
						}
						$("#cancel_tender_form_content").dialog("close"); 
			...
			$("#cancel_tender_form :button").prop('disabled',false);				
		})();
	});            
</script>