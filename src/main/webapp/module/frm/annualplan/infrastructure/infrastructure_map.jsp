...
function loadInfrastructureFeature(infrId){
	var url ='MUnitMapJson.infrastructureFeature.do?infrastructure_id=' +infrId;
	if(url){
		var request = $.ajax({
			type : 'GET', // GET or POST
			url : url,  
...
function onPlan(feature) {
	...
	$geometry_start = $("#activitypointstart");
	$geometry_end = $("#activitypointend");
	if($geometry_start && $geometry_end){
		var polygonFeature = wkt.read(wkt_obj);
         ...
         if( $("#activitypointstart").val()==''){
        	   $geometry_start.val(polygonFeature.geometry); 
         }else if( $("#activitypointend").val()==''){
        	 $geometry_end.val(polygonFeature.geometry); 
         }else{
        	 if( confirm("<s:text name='frm.stand.map.feature.delete.infr.point.msg'/>") ) { // prompt
                	vectorLayerPlan.removeFeatures( [ feature ] );

                }
         }
         if($("#activitypointstart").val()!='' && $("#activitypointend").val()!='' && $("#road\\.id").val()!=''){
        	 var rez=checkLinePoints($("#road\\.id").val(),$("#activitypointstart").val(), $("#activitypointend").val())
        	 if(rez!=''){
        		 alert(rez);
        	 }else{
	        	 getInfrastructureActivityDetails($("#road\\.id").val(),$("#activitypointstart").val(), $("#activitypointend").val());
...
function checkLinePoints(roadId,start_point, end_point){
	var resultPoint='';
	$.ajax({
		type : "GET", // GET or POST
		url : "Input.checkLinePoints.do?roadId="+roadId+"&activitypointstart="+start_point+"&activitypointend="+end_point, // the file to call
		async : false,
		beforeSend:function(jqXHR, settings) {
		},
		...
		success : function(data, status, jqXHR) { // on success..
			...
			if(!jQuery.isEmptyObject(data)){
				for(var i=0;i<data.length;i++){
					var start_point_is_min=data[i].start_point_is_min;
					if(start_point_is_min=='false'){
						resultPoint='<s:text name='infr.line.start.and.end.point.check'/>';
					}else{
						resultPoint='';
					}
				}
...
function getInfrastructureActivityDetails(roadId,start_point, end_point){
		$.ajax({
			type : "GET", // GET or POST
			url : "Input.getActivityGeomDetails.do?roadId="+roadId+"&activitypointstart="+start_point+"&activitypointend="+end_point, // the file to call
			async : false,
			beforeSend:function(jqXHR, settings) {
			},
			error : function(errorThrown, status, jqXHR) { // on error..
			...
			success : function(data, status, jqXHR) { // on success..
				...
				if(!jQuery.isEmptyObject(data)){
					for(var i=0;i<data.length;i++){
						$("#roadActivity\\.plStartM").val(data[i].lg1);
						$("#roadActivity\\.plEndM").val(data[i].lg2);
						$("#roadActivity\\.plLenght").val(data[i].distance_lg0_lg_1);
						$("#plLine").val(data[i].geom_act);
...
$.ajax({
	type : "GET", // GET or POST
	url : "FrmGrid.layerGisData.do?module=INFR", // the file to call
	async : false,
	beforeSend:function(jqXHR, settings) {
	},
	error : function(errorThrown, status, jqXHR) { // on error..
...
//                scales: [74153,50000,32957,21971,14648,9765,6510,4340,2893,1929,1286,857,572,381,254,169],
    ...
    var  slv_im = new OpenLayers.Layer.WMS(
		...
		var orto = new OpenLayers.Layer.WMS(
		 ...
		 var styleMap = new OpenLayers.StyleMap({pointRadius: 10,strokeColor: "red"});
      ...
      //ms
        ...
        if($("#roadActivity\\.id").val()=='' || $("#roadActivity\\.activityStatus\\.status").val()=='NEW'){
        controlEdit = {
          				...
          				vectorLayerPlan, 
          			    {
          			        ...
          			        eventListeners: {
          			            featurehighlighted: function overlay_delete(event) {
          			               ...
          			               var $geometry= $("#toDeleteComparePoint");
          			               var wkt_obj = wkt.write(feature);
          			           	   var polygonFeature = wkt.read(wkt_obj);
		          			       	polygonFeature.geometry.transform(
		          			  			   new OpenLayers.Projection("EPSG:900913"),
		          			        ...
		          			        if($("#toDeleteComparePoint").val()==$("#activitypointstart").val() ){
		          			        	$("#activitypointstart").val(''); 
		          			        }
		          			        if($("#toDeleteComparePoint").val()==$("#activitypointend").val() ){
		          			        	$("#activitypointend").val(''); 
		          			        }
                ...
                point: new OpenLayers.Control.DrawFeature(vectorLayerPlan,
