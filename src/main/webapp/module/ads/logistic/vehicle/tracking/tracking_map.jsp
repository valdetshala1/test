...
function getActualDevicePoint(deviceId){
// 	if(showActualPosition=='true'){
	$.ajax({
		type : "GET", // GET or POST
		url : "LogisticGrid.getActualTracPoint.do?deviceId="+deviceId, // the file to call
		async : false,
		...
		success : function(data, status, jqXHR) { // on success..
			for(var i=0;i<data.length;i++){
				actualPointGeom=data[i].actual_point;
				plate_number=data[i].plate_number;
				speed=data[i].speed;
				uniqueid=data[i].uniqueid;
				brand=data[i].brand;
				course=data[i].course;
			//	alert(plate_number);
	  		   $( "#p_activeDeviceInfo" ).empty();
		       var htmlString = "Vetura me targa: "+plate_number+" e brendit "+brand+" ka shpejtësi monentale "+speed ;
		       $( "#p_activeDeviceInfo" ).text( htmlString );
...
function getActualDeviceTracking(deviceId){
		$.ajax({
			type : "GET", // GET or POST
			url : "LogisticGrid.getActualTracPosition.do?deviceId="+deviceId, // the file to call
			async : false,
			beforeSend:function(jqXHR, settings) {
			},
			error : function(errorThrown, status, jqXHR) { // on error..
			...
			success : function(data, status, jqXHR) { // on success..
				for(var i=0;i<data.length;i++){
					...
					actualLineGeom=data[i].geom;
// 					alert(actualLineGeom);
...
$.ajax({
	type : "GET", // GET or POST
	url : "FrmGrid.layerGisData.do?module=DTRM", // the file to call
	async : false,
	beforeSend:function(jqXHR, settings) {
	},
	error : function(errorThrown, status, jqXHR) { // on error..
...
//    });
        ...
        trac = new OpenLayers.Layer.WMS(
            "Trac", '/geoserver/wms',
            {
                LAYERS: 'kfis_gis:tlog_trips',
                STYLES: '',
                format: format,
                tiled: true,
				transparent: true,
// 				CQL_FILTER:'id in ('+log_trips_ids+')',
                tilesOrigin : map_trac.maxExtent.left + ',' + map_trac.maxExtent.bottom
            },
            {
                buffer: 0,
  ...
  //      map_trac.addLayers([openstreetmap,trac]);

      //ms
        drawLayer();
		var layerArray=[openstreetmap,trac,];
...
//     map_trac.addLayer(trackPointLayer);
   ...
   function doGetFeatureInfoPopup(evt){
    ...
    debugger
    var wkt_f;
    $("#tracId").on('click',function(){
    	 if(showActualPosition==true){
	    	getActualDeviceTracking(deviceId);
	    	getActualDevicePoint(deviceId);
	    	if(actualPointGeom && actualLineGeom){
		    	wkt_f=new OpenLayers.Format.WKT().read(actualLineGeom);
		    	wkt_f=wkt_f.geometry.transform(RKS, map_trac.getProjectionObject());
		
		     	var wkt_f_point=new OpenLayers.Format.WKT().read(actualPointGeom);
		    	wkt_f_point=wkt_f_point.geometry.transform(RKS, map_trac.getProjectionObject());
		
		    	wkt_f.addPoint(wkt_f_point);
    		 ...
    		 trackLineLayer1.destroyFeatures();
